...
public class Folder2Test extends BaseTest {
    ...
    public void testDeleteFolder() throws InterruptedException {
        ...
        WebElement folder = getDriver().findElement(By.xpath
                ("//a[@class='jenkins-table__link model-link inside']/button[@class='jenkins-menu-dropdown-chevron']"));
        folder.sendKeys(Keys.RETURN);
        Thread.sleep(3000);

        WebElement deleteFolder = getDriver().findElement(By.xpath("//a[contains(@href,'delete')]//span"));
        deleteFolder.click();
        Thread.sleep(2000);

        WebElement deleteFolderButton = getDriver().findElement(By.xpath("//button[@name='Submit']"));
        deleteFolderButton.click();

        Assert.assertFalse(getDriver().findElement(By.id("page-body")).getText().contains(FOLDER_NAME));
    }
}
