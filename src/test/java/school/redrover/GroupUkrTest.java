...
public class GroupUkrTest extends BaseTest {
    ...
    public void testCreateFirstProject() {
        ...
        Assert.assertTrue(checkboxDropDown.get(0).isDisplayed());
        ...
        Assert.assertTrue(checkboxDropDown.get(3).isDisplayed() && checkboxDropDown.get(4).isDisplayed());
        checkboxGeneralSettings.get(3).click();
        checkboxDropDown = findCheckboxDropDownElements();
        actions.scrollToElement(checkboxDropDown.get(6)).perform();
        Assert.assertTrue(checkboxDropDown.get(5).isDisplayed() && checkboxDropDown.get(6).isDisplayed());
        actions.scrollToElement(namesOfBlockTitle.get(0)).perform();
        checkboxGeneralSettings.get(4).click();
        checkboxDropDown = findCheckboxDropDownElements();
        Assert.assertTrue(checkboxDropDown.get(7).isDisplayed());


        Assert.assertEquals(namesOfBlockTitle.get(0).getText(), "Управление исходным кодом");
        ...
        Assert.assertEquals(namesOfBlockTitle.get(1).getText(), "Триггеры сборки");
        ...
        actions.scrollToElement(namesOfBlockTitle.get(5)).perform();
        Assert.assertEquals(namesOfBlockTitle.get(2).getText(), "Среда сборки");
        Assert.assertEquals(namesOfBlockTitle.get(4).getText(), "Шаги сборки");
        Assert.assertEquals(namesOfBlockTitle.get(5).getText(), "Послесборочные операции");
        ...
        Assert.assertEquals(addStepsBuildDropDown.get(0).getText(), "Invoke Gradle script");
        Assert.assertEquals(addStepsBuildDropDown.get(1).getText(), "Run with timeout");
        Assert.assertEquals(addStepsBuildDropDown.get(2).getText(), "Set build status to \"pending\" on GitHub commit");
        Assert.assertEquals(addStepsBuildDropDown.get(3).getText(), "Вызвать Ant");
        Assert.assertEquals(addStepsBuildDropDown.get(4).getText(), "Вызвать цели Maven верхнего уровня");
        Assert.assertEquals(addStepsBuildDropDown.get(5).getText(), "Выполнить команду Windows");
        Assert.assertEquals(addStepsBuildDropDown.get(6).getText(), "Выполнить команду shell");
        ...
        Assert.assertEquals(addStepsAfterBuildDropDown.get(2).getText(), "Заархивировать артефакты");
        Assert.assertEquals(addStepsAfterBuildDropDown.get(3).getText(), "Собрать другой проект");
        Assert.assertEquals(addStepsAfterBuildDropDown.get(4).getText(), "Сохранять отпечатки файлов (fingerprints) для отслеживания использования");
        ...
        Assert.assertEquals(addStepsAfterBuildDropDown.get(9).getText(), "Уведомление по почте");
        ...
        try {
            ...
            wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[text()='Удалить Проект']"))).click();
